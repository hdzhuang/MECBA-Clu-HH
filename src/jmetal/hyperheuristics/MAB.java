...
public class MAB extends HyperHeuristicSelector{
    ...
    public MAB(double scalingFactor, ArrayList<AlgorithmHH> algs, int numObj) {
        super(algs, numObj);
    ...
    public double getReward(int op){
        ...
        AlgorithmRanking rniRanking = this.getMetric(algs.get(op), rni); //RNI
        //frequencies[i] = frequencies[i] + rniRanking.getRanking();//RNI
        
        double value = 2.0 * (double) (numberOfHeuristics+1) - (double) (frequencies[op] + rniRanking.getRanking());
        return (value-2.0)/(6.0-2.0);  
    }
